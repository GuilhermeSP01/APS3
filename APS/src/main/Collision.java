...
public class Collision {
	...
	public int checkDiscard(Player element, boolean player) {
		
		int index = 999;
		
		for(int i = 0; i < gp.discard.length; i++) {
			
			element.solidArea.x = element.x + element.solidArea.x;
			element.solidArea.y = element.y + element.solidArea.y;
			
			gp.discard[i].solidArea.x = gp.discard[i].x + gp.discard[i].solidArea.x;
			gp.discard[i].solidArea.y = gp.discard[i].y + gp.discard[i].solidArea.y;
			
			
			switch(element.direction) {
			case 0:
				element.solidArea.y -= element.speed;
				if(element.solidArea.intersects(gp.discard[i].solidArea)) {
					if(gp.discard[i].collision == true) {
			...
			element.solidArea.x = element.solidAreaDefaultX;
			element.solidArea.y = element.solidAreaDefaultY;
			gp.discard[i].solidArea.x = gp.discard[i].solidAreaDefaultX;
			gp.discard[i].solidArea.y = gp.discard[i].solidAreaDefaultY;
